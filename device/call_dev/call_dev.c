...
ssize_t call_read (struct file* filp, char* buf, size_t count, loff_t* f_pos) {
	printk ("call read -> buf : %08X, whence : %08X\n", buf, count);
	return 0x33;
}

ssize_t call_write (struct file* filp, char* buf, size_t count, loff_t* f_pos) {
	printk ("call write -> buf : %08X, whence : %08X\n", buf, count);
	return 0x43;
}
