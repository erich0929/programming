...
/**
 * The RegistryKey class represents a key in the registry. The class also
 * provides all of the native interface calls.
 * 
 * You should refer to the Windows Registry API documentation for the details of
 * any of the native methods. The native implementation performs almost no
 * processing before or after a given call, so their behavior should match the
 * API's documented behavior precisely.
 * 
 * Note that you can not open a subkey without an existing open RegistryKey.
 * Thus, you need to start with one of the top level keys defined in the
 * Registry class and open relative to that.
 * 
 * @version 3.1.3
 * 
 * @see com.ice.jni.registryx.Registry
 * @see com.ice.jni.registryx.RegistryValue
 */
...
public class RegistryKey {
	...
	/**
	 * This is the actual DWORD key that is returned from the Registry API. This
	 * value is <strong>totally opaque</strong> and should never be referenced.
	 */
	protected int hKey;
	...
	/**
	 * Used to indicate whether or not the key was created when method
	 * createSubKey() is called, otherwise false.
	 */
	...
	/**
	 * Get the name of this key. This is <em>not</em> fully qualified, which
	 * means that the name will not contain any backslashes.
	 * 
	...
	/**
	 * Determine if this key was opened or created and opened. The result can
	 * only be true if createSubKey() was called and the key did not exist, and
	 * the creation of the new subkey succeeded.
	 * 
	...
	public RegistryKey openSubKey(String subkey) throws NoSuchKeyException,
			RegistryException {
		return this.openSubKey(subkey, ACCESS_READ);
	}

	/**
	 * Create, and open, a Registry subkey of this key with WRITE access. If the
	 * key already exists, it is opened, otherwise it is first created and then
	 * opened.
	 * 
	 * @param subkey
	...
	/**
	 * Connect to the remote registry on <em>hostName</em>. This method will
	 * only work when invoked on a toplevel key. The returned value will be the
	 * same toplevel key opened from the remote host's registry.
	 * 
	 * @param hostName
	...
	/**
	 * Create a new subkey, or open the existing one. You can determine if the
	 * subkey was created, or whether an existing subkey was opened, via the
	 * wasCreated() method.
	 * 
	 * @param subKey
	...
	public native RegistryKey createSubKey(String subKey, String className,
			int access) throws RegistryException;

	/**
	 * Closes this subkey. You may chose to let the finalize() method do the
	 * close.
	 * 
	 * @exception RegistryException
	 *                Any valid registry API error.
	...
	public native void deleteSubKey(String subKey) throws NoSuchKeyException,
			RegistryException;
	...
	/**
	 * Guarentees that this key is written to disk. This method should be called
	 * only when needed, as it has a huge performance cost.
	 * 
	 * @exception RegistryException
	 *                Any valid registry API error.
	 */
	...
	public native String getDefaultValue() throws NoSuchValueException,
			RegistryException;
	...
	/**
	 * This method will expand a string to include the definitions of System
	 * environment variables that are referenced via the %variable% construct.
	 * This method invokes EnvExpandStrings().
	 * 
	 * @param valueName
	 *            The name of the value to increment.
	 */
	...
	/**
	 * Returns a new Enumeration that will enumerate the names of the subkeys of
	 * this key,
	 * 
	...
	/**
	 * Returns a new Enumeration that will enumerate the names of the values of
	 * this key,
	 * 
	...
	/**
	 * A RegistryKey enumerator class. This enumerator is used to enumerate the
	...
	/**
	 * A RegistryValue enumerator class. This enumerator is used to enumerate
	 * the names of this key's values. This will return the default value name
	 * as an empty string.
	 * 
	 * This class should remain opaque to the client. It will use the
	 * Enumeration interface.
	 */
	...
	/**
	 * Export this key's definition to the provided PrintWriter. The resulting
	 * file can be imported via RegEdit.
	 * 
	 * @exception NoSuchKeyException
