...
//	private static RegistryKey HKEY_ICE_TESTKEY = null;
	...
	static {
		...
		Registry.HKEY_CLASSES_ROOT = new RegistryKey(0x80000000,
				"HKEY_CLASSES_ROOT");

		Registry.HKEY_CURRENT_USER = new RegistryKey(0x80000001,
				"HKEY_CURRENT_USER");

		Registry.HKEY_LOCAL_MACHINE = new RegistryKey(0x80000002,
				"HKEY_LOCAL_MACHINE");

		Registry.HKEY_USERS = new RegistryKey(0x80000003, "HKEY_USERS");

		Registry.HKEY_PERFORMANCE_DATA = new RegistryKey(0x80000004,
				"HKEY_PERFORMANCE_DATA");

		Registry.HKEY_CURRENT_CONFIG = new RegistryKey(0x80000005,
				"HKEY_CURRENT_CONFIG");
		...
		topLevelKeys.put("HKEY_PERFORMANCE_DATA",
	...
	public static RegistryKey openSubkey(RegistryKey topKey, String keyName,
			int access) {
	...
	public static void exportRegistryKey(String pathName, RegistryKey key,
			boolean descend) throws java.io.IOException, NoSuchKeyException,
			RegistryException {
...
//		try {
//			Registry.HKEY_ICE_TESTKEY = Registry.HKEY_CURRENT_USER
//					.openSubKey("Software\\ICE Engineering\\test");
...
//				LoggerModel.log("command: ");
	...
	public static void usage(String message) {
		...
		System.err
				.println("setbin regKey subKey  binaryString -- set REG_BINARY");
		System.err.println("setdw regKey subKey int -- set REG_DWORD");
		System.err.println("setstr regKey subKey string -- set REG_SZ");
		System.err
				.println("setmulti regKey subKey semiColonString -- set REG_MULTI_SZ");
		System.err
				.println("delkey regKey subKey -- delete key 'subKey' of regKey");
		System.err
				.println("delval regKey subKey -- delete value 'subKey' of regKey");
		System.err
				.println("export regKey fileName -- export registry key to fileName");
...
//		RegistryKey subKey;
		...
		if (Registry.saveKey != null && keyName.equals("$$")) {
			...
			if (ch1 == '$' && ch2 >= '0' && ch2 <= '9') {
				...
				if (Registry.preDefines[pIdx] != null) {
					...
					System.err.println("Predefine '" + keyName
							+ "' not defined.");
					return;
				}
			} else {
		...
		if (index < 0) {
			...
			//
			System.err.println("Invalid key '" + keyName
		...
		if (topKey == null) {
			System.err.println("ERROR, toplevel key '" + topKeyName
					+ "' not resolved!");
			return;
		}
		...
		if (isRemote) {
			...
			try {
				...
				System.err.println("ERROR No such key connecting to '"
						+ hostName + "', " + e.getMessage());
				return;
			} catch (RegistryException e) {
				System.err.println("ERROR errCode=" + e.getErrorCode()
	...
	private static void exportKeyCommand(RegistryKey topKey, String keyName,
			String pathName) {
		RegistryKey subKey = Registry.openSubKeyVerbose(topKey, keyName,
		...
		try {
			...
			System.err
					.println("Error, encountered non-existent key during export.");
		} catch (RegistryException ex) {
			System.err.println("ERROR registry error=" + ex.getErrorCode()
					+ ", " + ex.getMessage());
		}
	}

	private static void getDataCommand(RegistryKey topKey, String keyName,
			String valueName) {
		RegistryKey subKey = Registry.openSubKeyVerbose(topKey, keyName,
				RegistryKey.ACCESS_READ);
		...
		if (data instanceof RegStringValue) {
			...
			System.err
					.println("REG_DWORD"
							+ ((RegistryValue.REG_DWORD_BIG_ENDIAN == val
									.getType()) ? "_BIG_ENDIAN" : "") + " '"
							+ val.getData() + "' [x"
			 ...
			 */
			Registry.dumpHexData(System.err, "REG_BINARY '" + val.getName()
					+ "', len=" + val.getLength(), val.getData(), val
					.getLength());
		}
	}

	private static void getStringCommand(RegistryKey topKey, String keyName,
			String valueName) {
		RegistryKey subKey = Registry.openSubKeyVerbose(topKey, keyName,
				RegistryKey.ACCESS_READ);
		...
		try {
			...
			System.err
					.println("String Value " + valueName + "='" + value + "'");
		} catch (RegistryException ex) {
			System.err.println("ERROR getting value '" + valueName + "', "
					+ ex.getMessage());
			return;
		}
	}

	private static void expandStringCommand(RegistryKey topKey, String keyName,
			String valueName) {
		RegistryKey subKey = Registry.openSubKeyVerbose(topKey, keyName,
				RegistryKey.ACCESS_READ);
	...
	private static void deleteKeyCommand(RegistryKey topKey, String keyName,
		...
		try {
			...
			System.err.println("Key '" + keyName + "\\" + deleteKeyName
					+ "' does not exist.");
			return;
		} catch (RegistryException ex) {
			System.err.println("ERROR deleting key '" + keyName + "', "
					+ ex.getMessage());
			return;
		}
	}

	private static void deleteValueCommand(RegistryKey topKey, String keyName,
			String valueName) {
		RegistryKey subKey = Registry.openSubKeyVerbose(topKey, keyName,
				RegistryKey.ACCESS_WRITE);
		...
		try {
			...
			System.err.println("ERROR deleting value '" + valueName + "', "
					+ ex.getMessage());
			return;
		}
	}
	...
	private static void listKeysCommand(RegistryKey topKey, String keyName) {
		...
		try {
			Enumeration e = subKey.keyElements();
			for (int kIdx = 0; e.hasMoreElements(); ++kIdx) {
				String keyStr = (String) e.nextElement();
				System.err.println("Subkey[" + kIdx + "] = '" + keyStr + "'");
			}
		} catch (RegistryException ex) {
			System.err.println("ERROR getting key enumerator, "
					+ ex.getMessage());
			return;
		}
	}
	...
	private static void listValuesCommand(RegistryKey topKey, String keyName) {
		...
		try {
			Enumeration e = subKey.valueElements();
			for (int kIdx = 0; e.hasMoreElements(); ++kIdx) {
				String name = (String) e.nextElement();
				System.err.println("Value Name[" + kIdx + "] = '" + name + "'");
			}
		} catch (RegistryException ex) {
			System.err.println("ERROR getting value enumerator, "
					+ ex.getMessage());
			return;
		}
	}

	private static void setDWordCommand(RegistryKey topKey, String keyName,
	...
	private static void setMultiStringCommand(RegistryKey topKey,
			String keyName, String valueName, String data) {
		RegistryKey subKey = Registry.openSubKeyVerbose(topKey, keyName,
				RegistryKey.ACCESS_WRITE);
		...
		RegMultiStringValue val = new RegMultiStringValue(subKey, valueName,
				strArray);

		Registry.setValue(subKey, val);
	}

	private static void setStringCommand(RegistryKey topKey, String keyName,
			String valueName, String data) {
		RegistryKey subKey = Registry.openSubKeyVerbose(topKey, keyName,
				RegistryKey.ACCESS_WRITE);
	...
	private static void setBinaryCommand(RegistryKey topKey, String keyName,
			String valueName, String data) {
		RegistryKey subKey = Registry.openSubKeyVerbose(topKey, keyName,
				RegistryKey.ACCESS_WRITE);
	...
	private static void createCommand(RegistryKey topKey, String keyName) {
		...
		if (subKey != null) {
			try {
				...
				subKey = null;
				System.err.println("ERROR flushing and closing key: "
						+ ex.getMessage());
			}
		}
		...
		if (subKey != null) {
			System.err.println("SUCCEEDED "
					+ (subKey.wasCreated() ? "Creating" : "Opening via create")
					+ " Key '" + keyName + "'");
		} else {
	...
	private static void setValue(RegistryKey subKey, RegistryValue value) {
		try {
			...
			System.err.println("ERROR setting MULTI_SZ value '"
