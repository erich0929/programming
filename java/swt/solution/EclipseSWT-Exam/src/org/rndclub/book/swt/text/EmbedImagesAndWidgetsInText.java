...
public class EmbedImagesAndWidgetsInText {
	...
	public static void main(String[] args) {
		...
		final Image[] images = { new Image(display, 32, 32),
				new Image(display, 20, 40), new Image(display, 40, 20) };
		...
		shell.addListener(SWT.Paint, new Listener() {
			public void handleEvent(Event event) {
				...
				for (int i = 0; i < images.length; i++) {
					...
					gc.drawImage(images[i], point.x + margin.x, point.y
							+ margin.y + lineMetrics.getAscent()
							- glyphMetrics.ascent);
				}
				...
				button.setLocation(point.x + margin.x, point.y + margin.y
						+ lineMetrics.getAscent() - glyphMetrics.ascent);
			}
		});
