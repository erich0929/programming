package org.rndclub.book.swt.text;
...
public class ChangeRiseOfText {
	...
	public static void main(String[] args) {
		...
		final int smallBaseline = smallMetrics.getAscent()
				+ smallMetrics.getLeading();
		gc.setFont(font);
		...
		shell.addListener(SWT.Paint, new Listener() {
			public void handleEvent(Event event) {
				...
				gc.drawLine(rect0.x, rect0.y + baseline, rect0.x + rect0.width,
						rect0.y + baseline);
				gc.drawLine(rect0.x + rect0.width / 2, rect0.y, rect0.x
						+ rect0.width / 2, rect0.y + rect0.height);
				...
				gc.setForeground(display.getSystemColor(SWT.COLOR_MAGENTA));
				gc.drawLine(rect1.x, rect1.y + smallBaseline, rect1.x
						+ rect1.width, rect1.y + smallBaseline);
				gc.drawLine(rect1.x, rect1.y + baseline + smallBaseline,
						rect1.x + rect1.width, rect1.y + baseline
								+ smallBaseline);
				gc.drawLine(rect1.x + rect1.width / 2, rect1.y, rect1.x
						+ rect1.width / 2, rect1.y + rect1.height);
			}
		});
		shell.open();
